module Gandi
  ...
  class Domain
    ...
    def price(domain, *args)
      ...
      Rails.logger.debug { "Domain price lookup: #{product_spec.inspect}" }
      result = catalog(product_spec, *args)
      Rails.logger.debug { "Price result: #{result.inspect}" }
    ...
    def search(domains)
      ...
      Rails.logger.debug { "Searching domains: #{domains.inspect}" }
    ...
    def create(domain)
      ...
      @contact = contact
      Rails.logger.debug { "Setting Gandi contact: #{@contact}" }
      domain_spec = {
        owner: @contact,
        admin: @contact,
        bill: @contact,
    ...
    def contact
      if Rails.env == 'development'
        ...
        Rails.logger.debug { "Create contact returned: #{result.inspect}" }
